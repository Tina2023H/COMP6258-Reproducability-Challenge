...
class KITTI(Dataset):
    def __init__(self, opt: DictConfig, partition: str) -> None:
        ...
        See https://www.cvlibs.net/datasets/kitti/ for more information about the dataset.
        ...
        self.img_dir = Path(opt.cwd, opt.input.load_path, partition, "image")
        self.img_files = [
            name for name in os.listdir(self.img_dir) if name.endswith(".png")
        ]
        self.ann_dir = Path(opt.cwd, opt.input.load_path, partition, "semantic")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        pixelwise_class_label = self.to_tensor(
            PILImage.open(self.ann_dir / self.img_files[idx])
        )
        ...
        labels = {
            ...
            "pixelwise_instance_labels": pixelwise_class_label,
        }
